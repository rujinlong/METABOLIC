...
#
#	 Modified by: Zhichao Zhou; zczhou2017@gmail.com; July, 2019
#
#    Original address: http://github.com/minillinim/gff2fasta
#
...
while(<$gff_fh>){
    ...
    ($attributes) = ($attributes =~ m/(ID|Alias)=([^;]*)/i)[1]; #print "$attributes\n";
...
while(my $sobj = $seqio->next_seq)
{
    ...
    if(defined($global_gff_orfs{$seqid}))
    {
        for(@{$global_gff_orfs{$seqid}})
        {
			...
			my ($attributes_2ndpart) = $attributes =~ /(\_\d+?)$/;
		...
		{
		    print $out_fh ">$seqid"."$attributes_2ndpart\n";
		}
